...
function Get-VeracodeScaWorkspaceID {
    ...
    $apiReturn = http --auth-type=veracode_hmac "https://api.veracode.com/srcclr/v3/workspaces?filter[workspace]=$workspaceName" | ConvertFrom-Json
...
function New-VeracodeWorkspace {
    param (
        [Parameter(Mandatory=$true)]
        [string]$workspaceName,
        [string]$teamName
    )
    Write-Host "New Workspace Name: $workspaceName"

    # Define the JSON content with the placeholder "string"
    $jsonContent = @"
{
  "name": "$teamName"
}
"@
    ...
    get-content $jsonContent | Out-File -Path teams.json
    Write-Host "JSON content written to teams.json"
    if ($teamName) {
            http --auth-type=veracode_hmac POST "https://api.veracode.com/srcclr/v3/workspaces" @'teams.json' | ConvertFrom-Json
