...
install_veracode_cli(){
    if program_exists "veracode"; then # check to see if the program exists, if it does
        if [ "$DEBUG" == "true" ]; then # let the user know it is already in the execution path 
            echo "[DEBUG=$DEBUG]:: veracode cli already exists in the execution path."
        fi
        echo "Veracode-Cli Location: $( which veracode )"   # specify where
    else
        echo "[INFO]:: Veracode CLI wasn't found in the execution path, installing..." # if it wasn't found in the execution path start running the install
        curl -fsS https://tools.veracode.com/veracode-cli/install | sh
...
# Postcondition:
main() {
    # check to see if the flag to install the cli is on
    if [ "$install_cli" == "true" ]; then
        install_veracode_cli    # if it is on check to see if need to install the cli
    fi
