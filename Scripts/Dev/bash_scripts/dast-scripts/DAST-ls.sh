...
help(){
    ...
    echo "  -s  --silent     silent, turn off output"
    echo "  -v  --verbose    verbose, makes additional API calls to pull the analysis occurrence status"
    echo "  -a       List out analysis details [Default]"
}

# #veracode hmac authentication alternative to using the httpie and python authentication library
# make_call(){
#     API_ID=$1
#     API_KEY=$2
