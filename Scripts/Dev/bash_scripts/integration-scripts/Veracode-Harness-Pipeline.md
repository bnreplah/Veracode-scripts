# Veracode Harness Pipeline #

## How to use ##

1) Set your `VERACODE_API_KEY_ID` and `VERACODE_API_KEY_SECRET` in the secrets in Harness, do the same with your SRCCLR token for the SCA agent
2) Add a Run Step in your harness pipelines and select bash as the script option. Then paste in the Harness pipeline
3) The pipeline has the default behavior to leverage a sandbox scan with the following name: `sandbox-$DRONE_REPO_NAME-<+codebase.branch>-<+pipeline.name>` and uses the `$DRONE_REPO_NAME` to determine the app profile to submit to.
4) It will create a new sandbox if one doesn't exist, however it will not create a new Application profile if it doesn't exist.
5) There is a feature to pull down a list of applications from the platform to cross reference those as well in a function left for use.



Setting the `PIPELINE_SCAN` variable at the top will cause it to revert to leverage a pipeline scan instead
> You can optionally remove or comment out this variable and place it in the enviornment variables in the Harness pipeline to control it from there.  
> The same is the case with the `DEBUG` parameter

In the case that you need to import results into the Harness STO, you can pull down a similiar report depending on the input that Harness STO takes
The `DetailReport` `SummaryReport` actions can also be used to pull down reports around the scan

You can either have the task all in one, or broken up into multiple
...
In order to set your secrets you need to click on `Project Settings`
...
Set the `Veracode API ID` and `Veracode API KEY` from the platform 
